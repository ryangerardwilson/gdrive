...
FILENAME="gdrive-linux-x64.tar.gz"
...
write_public_launcher() {
  ...
  elif [[ -f "$PUBLIC_LAUNCHER" ]] && ! grep -Fq "exec \"${INSTALL_DIR}/${APP}\" \"\$@\"" "$PUBLIC_LAUNCHER" 2>/dev/null; then
...
#!/usr/bin/env bash
# Managed by gdrive local-bin launcher
set -euo pipefail
